...
module "prm-process-not-started-alerts" {
  ...
  alert_desc = "Triggers when ${each.value.description} has not started within the past hour in am-${var.env}."
  app_insights_query = "customEvents | where name == \"${each.value.notStartedQueryFilter}\" | limit 1"
  custom_email_subject = "Alert: ${each.value.description} has not started in expected time frame in am-${var.env}"
