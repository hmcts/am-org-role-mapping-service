...
Feature: Create Role Assignments for JRD Users
  ...
  Scenario: must successfully create judicial role mapping for a multiple user having single role
    ...
    And a successful call [to delete existing role assignments corresponding to the test actorId] as in [S-001_DeleteDataForRoleAssignments01],
    And a successful call [to delete existing role assignments corresponding to the test actorId] as in [S-001_DeleteDataForRoleAssignments02],
    And a successful call [to publish an existing JRD user ids to topic] as in [S-001_PushMessageToAzureServiceBus],
