...
Feature: Create Role Assignments for Users
  ...
  Scenario: must successfully create org role mapping for CA and TS roles
    ...
    And a successful call [to delete existing role assignments corresponding to the test actorId] as in [S-010_DeleteDataForRoleAssignments02],
    And a successful call [to publish an existing CRD user ids to topic] as in [S-010_PushMessageToAzureServiceBus],
    And a successful call [to provide adequate time for RAS to create records] as in [WaitForRASProcessing],
