...
#  Scenario: must successfully create org role mapping for a single user having single role
#    Given a user with [an active IDAM profile with full permissions],
#    And a successful call [to delete existing role assignments corresponding to the test actorId] as in [S-001_DeleteDataForRoleAssignments],
#    And a successful call [to publish an existing CRD user ids to topic] as in [PushMessageToAzureServiceBus],
