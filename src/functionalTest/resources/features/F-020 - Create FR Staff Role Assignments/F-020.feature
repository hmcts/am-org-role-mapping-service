...
Feature: F-020 : Create Financial Remedy Staff Role Assignments
  ...
  @Ignore
  @S-020.DIAG
  Scenario: diagnostic create Financial Remedy caseworker profile in CRD
    Given a user with [an active IDAM profile with full permissions],
    When a request is prepared with appropriate values,
    And it is submitted to call the [Create Caseworker Profile in CRD] operation of [Caseworker Reference Data],
    Then a positive response is received,
    And the response has all other details as expected
