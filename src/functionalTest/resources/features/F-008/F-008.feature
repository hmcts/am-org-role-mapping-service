...
Feature: F-008 :Create Role Assignments for PrivateLaw Caseworker and Judicial Users
  ...
  Scenario: must successfully create judicial role mapping for circuit-judge - fee paid appointment.
    Given a user with [an active IDAM profile with full permissions],
    And a successful call [to delete existing role assignments corresponding to the test actorId] as in [S-073_DeleteDataForRoleAssignments01],
  ...
  @S-074
  @FeatureToggle(DB:privatelaw_wa_1_0=on)
  Scenario: must successfully create judicial role mapping for hearing-judge - salaried appointment.
    Given a user with [an active IDAM profile with full permissions],
    And a successful call [to delete existing role assignments corresponding to the test actorId] as in [S-074_DeleteDataForRoleAssignments01],
