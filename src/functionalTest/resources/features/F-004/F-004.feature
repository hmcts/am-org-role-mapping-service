...
Feature: F-004 : Refresh Role Assignments for Judicial Users
  ...
  Scenario: must successfully create judicial role mappings for User having no judicial booking
    Given a user with [an active IDAM profile with full permissions],
    And a successful call [to delete existing role assignments corresponding to the test actorId] as in [S-033_DeleteDataForRoleAssignments01],
    And a successful call [to publish existing JRD user ids to endpoint] as in [S-033_PushMessageToJRDService],
    ...
    And the request [contains the actorId who has no judicial booking],
    When a request is prepared with appropriate values,
    And it is submitted to call the [Fetch Assignment From Role Assignment Service] operation of [Role Assignment Service],
    Then a positive response is received,
    And the response has all other details as expected,
