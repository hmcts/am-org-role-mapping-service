...
Feature: F-009 : Create Role Assignments for PublicLaw Caseworker and Judicial Users
  ...
  @S-094
  @FeatureToggle(DB:publiclaw_hearing_1_0=on) @FeatureToggle(DB:publiclaw_wa_2_1=on) @FeatureToggle(EV:JUDICIAL_FTA_ENABLED=on)
  Scenario: must successfully create judicial role mapping for Deputy District Judge - Fee-Paid appointment
    ...
    And a successful call [to snapshot judicial details] as in [S-094__SnapshotJudicialDetails],
  ...
  Scenario: must successfully create judicial role mapping for Circuit Judge - Salaried appointment
    Given a user with [an active IDAM profile with full permissions],
    ...
    And a successful call [to snapshot judicial details] as in [S-095__SnapshotJudicialDetails],
