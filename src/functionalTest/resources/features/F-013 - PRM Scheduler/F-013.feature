...
Feature: F-013 : PRM Scheduler
  ...
  @S-013.06a
  @FeatureToggle(EV:PRM_FTA_ENABLED=on)
  Scenario: trigger PRM scheduler process 6 - Refresh Caseworkers
    Given a user with [an active IDAM profile with full permissions],
    When a request is prepared with appropriate values,
    And it is submitted to call the [Refresh Caseworkers] operation of [Organisation Role Mapping],
    Then a positive response is received,
    And the response has all other details as expected.
