...
Feature: F-005 : Create Role Assignments for Hearing Roles
  ...
  Scenario: must successfully create organisational role mapping for listed-hearing-viewer
    ...
    And a successful call [to delete existing role assignments corresponding to the test actorId] as in [S-41_DeleteDataForRoleAssignments]
  ...
  Scenario: must successfully create organisational role mapping for hearing-viewer and hearing-manager
    Given a user with [an active IDAM profile with full permissions],
    And a successful call [to delete existing role assignments corresponding to the test actorId] as in [S-42_DeleteDataForRoleAssignments]
