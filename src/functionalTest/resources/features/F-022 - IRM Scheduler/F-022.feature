...
Feature: F-022 : IRM Scheduler
  ...
  Scenario: trigger IRM scheduler process empty judicial queue
    ...
    And a successful call [to make the IRM queue value inactive] as in [F-022-MakeAllIrmQueueValuesInactive]
  ...
  Scenario: trigger IRM scheduler process populated judicial queue
    Given a user with [an active IDAM profile with full permissions],
    And a successful call [to make the IRM queue value inactive] as in [F-022-MakeAllIrmQueueValuesInactive]
    And a successful call [to make the IRM queue value active] as in [S-022.06_MakeIrmQueueValue1Active]
    And a successful call [to make the IRM queue value active] as in [S-022.06_MakeIrmQueueValue2Active]
    When a request is prepared with appropriate values,
