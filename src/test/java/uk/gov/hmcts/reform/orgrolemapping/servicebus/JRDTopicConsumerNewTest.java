...
class JRDTopicConsumerNewTest {
    ...
    void shouldProcessMessage() throws Exception {
        ...
        // THEN
        verify(topicConsumer, times(1)).processMessage(serviceBusReceivedMessageContext, UserType.JUDICIAL);
        verify(bulkAssignmentOrchestrator, times(1)).createBulkAssignmentsRequest(userRequest, UserType.JUDICIAL);
    }
    ...
    void shouldThrowExceptionWhenMessageReceivedWithIncorrectFormat() throws InterruptedException {
        ...
        verify(bulkAssignmentOrchestrator, never()).createBulkAssignmentsRequest(any(), eq(UserType.JUDICIAL));
    }
