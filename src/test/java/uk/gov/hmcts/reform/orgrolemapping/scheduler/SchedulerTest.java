...
class SchedulerTest {
    ...
    void processUserRefreshQueue_start_and_mark_success_and_completed_when_all_entities_are_succesful() {
        ...
        when(professionalUserService.findAndInsertUsersWithStaleOrganisationsIntoRefreshQueue(any()))
                .thenReturn(true);

        // act
        scheduler.findUsersWithStaleOrganisationsAndInsertIntoRefreshQueueProcess();
    ...
    void processUserRefreshQueue_start_and_mark_partial_success_and_completed_when_some_entities_are_successful() {
        // arrange
        when(organisationRefreshQueueRepository.getActiveOrganisationRefreshQueueCount())
                .thenReturn(2L).thenReturn(1L).thenReturn(0L);
        when(professionalUserService.findAndInsertUsersWithStaleOrganisationsIntoRefreshQueue(any()))
